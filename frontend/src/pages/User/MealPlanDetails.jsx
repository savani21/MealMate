...
import { ArrowLeft, Sparkles, Utensils, ShoppingBasket } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#f7faf7]">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          {plan.meals.map((day) => (
            <div key={day.day} className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 md:p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <MealCard title="Breakfast" meal={day.breakfast} recipe={findRecipe(day.breakfast)} onRecipe={() => openRecipe(day.breakfast)} onGenerate={() => generateRecipe(day.breakfast)} />
                <MealCard title="Lunch" meal={day.lunch} recipe={findRecipe(day.lunch)} onRecipe={() => openRecipe(day.lunch)} onGenerate={() => generateRecipe(day.lunch)} />
                <MealCard title="Snack" meal={day.snack} recipe={findRecipe(day.snack)} onRecipe={() => openRecipe(day.snack)} onGenerate={() => generateRecipe(day.snack)} />
                <MealCard title="Dinner" meal={day.dinner} recipe={findRecipe(day.dinner)} onRecipe={() => openRecipe(day.dinner)} onGenerate={() => generateRecipe(day.dinner)} />
              </div>
            </div>
          ))}
        </section>
        ...
        <div className="flex justify-center mt-8">
          <button onClick={generateGroceryList} disabled={groceryLoading} className="flex items-center gap-2 px-6 py-3 rounded-xl bg-primary text-white font-bold hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed">
...
function MealCard({ title, meal, recipe, onRecipe, onGenerate }) {
  return (
    <div className="border border-gray-100 rounded-2xl p-5">
      ...
      <button onClick={onGenerate} className="mt-4 w-full py-2.5 rounded-xl bg-primary text-white text-sm font-semibold hover:opacity-90 transition">
        Generate Recipe
      </button>

      {recipe && (
        <button onClick={onRecipe} className="mt-2 w-full py-2.5 rounded-xl border border-gray-200 bg-white text-gray-700 text-sm font-semibold hover:border-primary hover:text-primary transition">
